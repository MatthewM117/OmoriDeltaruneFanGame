...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] GameObject sunnyHurt;
    [SerializeField] GameObject sunnyNeutral;
    [SerializeField] GameObject sunnyAfraid;
    ...
    void Update()
    {
        ...
        {
            movement.x = Input.GetAxisRaw("Horizontal") * platformerSpeed;
        }
        ...
        else if (Input.GetKeyUp(KeyCode.UpArrow) && isJumping)
        {
            isJumping = false;
            if (rb.velocity.y > 0)
            {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("obstacle"))
        {
            ...
            if (IsAfraid())
            {
                StartCoroutine(DecreaseHealth(Random.Range(60, 100)));
            }
            else
            {
                StartCoroutine(DecreaseHealth(Random.Range(30, 60)));
            }
    ...
    {
        sunnyAnimator.Play("sunny_hit", 0, 0f);
        AudioManager.instance.PlayAudio("heart_hit");
        sunnyHurt.SetActive(true);
        sunnyNeutral.SetActive(false);

        // game over
        if ((playerHealth.value - damage <= 0 && !GlobalData.instance.is2Player) || (playerHealth.value - damage <= 0 && GlobalData.instance.is2Player && gm.onePlayerDead))
        {
            hitDebounce = true;
        ...
        else if (playerHealth.value - damage <= 0 && GlobalData.instance.is2Player)
        {
            hitDebounce = true;
            disableMovement = true;
            if (isPlatformer)
        ...
        {
            sunnyAfraid.SetActive(true);
            emotionText.text = "Afraid";
        }
        else
        {
            sunnyNeutral.SetActive(true);
        }
        sunnyHurt.SetActive(false);
    }
